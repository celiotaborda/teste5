// Adiciona o mapa base do Google Satellite
var googleSatellite = new ol.layer.Tile({
    title: 'Google Satellite',
    type: 'base',
    visible: true,
    displayInLayerSwitcher: false, // Oculta da legenda
...
var map = new ol.Map({
    target: 'map',
    renderer: 'canvas',
    layers: [googleSatellite].concat(layersList),
    view: new ol.View({
...
// Desativa todas as camadas NDRE inicialmente
var ndreGroup = map.getLayers().getArray().find(layer => 
    layer instanceof ol.layer.Group && layer.get('title') === 'NDRE'
);

if (ndreGroup) {
    ndreGroup.getLayers().forEach(layer => {
        layer.setVisible(false);
    });
}

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([495274.584608, 6711938.126734, 511934.190370, 6721628.483221], map.getSize());
...
// Organiza as camadas NDRE em ordem cronológica
var ndreGroup = map.getLayers().getArray().find(layer => 
    layer instanceof ol.layer.Group && layer.get('title') === 'NDRE'
);

if (ndreGroup) {
    var layers = ndreGroup.getLayers().getArray();
    layers.sort((a, b) => {
        // Extrai a data do título da camada (formato: DD-MM)
        const getDate = (layer) => {
            const match = layer.get('title').match(/(\d{2})-(\d{2})/);
            if (match) {
                const [_, day, month] = match;
                // Assumindo o ano 2024 para datas até setembro e 2025 para o resto
                const year = parseInt(month) <= 9 ? 2024 : 2025;
    ...
    // Atualiza as camadas ordenadas
    ndreGroup.setLayers(new ol.Collection(layers));
}